...
export const projects: Project[] = [
  ...
  },
  {
    ...
    sections: {
      ...
      developmentNote: [
        "AI设计评审实验室是一个面向设计类学生的智能评审平台，支持海报、展板、模型图、UI界面及作品集等多种内容分析。系统能够从创意、逻辑、视觉表达、专业完成度和商业价值等维度进行评分，并生成优缺点与修改建议，帮助学生在课程提交、竞赛和求职前获得更及时、结构化的反馈。",
        "项目开发过程中主要遇到了评分结果不稳定、不同层级优秀作品区分不准确，以及通用评审标准难以适配多个设计专业等问题。针对这些困难，我通过降低模型随机性、固定评分流程、在后端重新计算总分、加入行业基准校准，并采用“通用维度＋专业扩展维度”的方式持续优化，使评审结果更加稳定、合理且适用于不同类型的设计作品。"
      ],
      process: [
